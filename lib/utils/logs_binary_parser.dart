...
///
/// Supports two packet sizes:
/// - **64 bytes** — original firmware (61 bytes data + 3 bytes padding)
/// - **61 bytes** — newer firmware (no padding)
///
/// The packet size is auto-detected from the first two valid headers.
class BinaryParser {
  /// Padded packet size (original firmware).
  static const int packetSize = 64;

  /// Data-only packet size (no padding).
  static const int dataSize = 61;

  /// Iterates through [rawBytes] and extracts [SensorLog] objects.
  ///
  /// Auto-detects whether packets are 61 or 64 bytes by examining
  ...
  ///
  /// ### Packet Structure (61 bytes data, optionally + 3 padding)
  ...
  static int _detectPacketSize(Uint8List rawBytes) {
    ...
    for (var i = 0; i + dataSize <= rawBytes.length; i++) {
      ...
      // Unexpected gap — keep scanning
    }

    // Fallback: check if file divides evenly by 61
